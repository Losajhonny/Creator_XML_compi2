 ...
 * @author Jhona
 */
public class Todos extends FunNativaArreglo implements Expresion, Instruccion {
    
    public Todos(String id, int line, int colm) {
        super(id, line, colm);
    }
    ...
    public Object execute(Entorno ent)
    {
        if(arr != null)
        {
            if(arr.tam > 0)
            {
                ...
                Literal tmp = new Literal(Tipo.BOOLEANO, Operacion.getBooleano(1), line, colm);
                ...
                for (Literal val : valores)
                {
                    ...
                    if(res.tipo == Operacion.Tipo.BOOLEANO)
                    {
                        ...
                        if(v == 0) { return res; } else { tmp = res; }
                    }
                }
                return tmp;
            }
